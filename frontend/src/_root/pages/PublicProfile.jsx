...
const PublicProfile = () => {
   ...
   const { privacy, profile, academic, activity, contact, relationship } = userData
         ...
         <div className="space-y-6">
                     ...
                     <div className="flex-1 space-y-2">
                        ...
                        <div className="flex flex-wrap gap-4 text-sm text-gray-600">
                           ...
                           <div className="flex items-center gap-1">
                              ...
                              Joined {new Date(userData.account?.createdAt).toLocaleDateString('en-US', {
                                 year: 'numeric',
                                 month: 'long'
                              })}
                           </div>
               ...
               <Card>
                  <CardContent className="p-6 text-center">
                     ...
                     <p className="text-gray-600">
                        This user has a private profile. {canFollow ? 'Follow them to see their activity.' : 'Only they can see their profile details.'}
                     </p>
                  </CardContent>
               </Card>
            )}
            ...
            {!hasAccess && privacy?.profileVisibility === 'university' && (
               <Card>
                  <CardContent className="p-6 text-center">
                     ...
                     <h3 className="text-lg font-semibold mb-2">University-Only Profile</h3>
                     <p className="text-gray-600">
                        This profile is only visible to students from {academic?.university}.
                        {clerkUser ? ' Make sure your university is set in your profile settings.' : ' Please sign in to view university profiles.'}
                     </p>
                  </CardContent>
               </Card>
            )}
            ...
            {hasAccess && (
               <>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                     <Card>
                        <CardContent className="p-6 text-center">
                           ...
                           <p className="text-2xl font-bold">{activity?.totalUploads || 0}</p>
                     ...
                     <Card>
                        <CardContent className="p-6 text-center">
                           ...
                           <p className="text-2xl font-bold">{activity?.totalDownloads || 0}</p>
                     ...
                     <Card>
                        <CardContent className="p-6 text-center">
                           ...
                           <p className="text-2xl font-bold">{activity?.totalLikesReceived || 0}</p>
                     ...
                     <Card>
                        <CardContent className="p-6 text-center">
                           ...
                           <p className="text-2xl font-bold">{activity?.followersCount || 0}</p>
                  ...
                  {/* Social Links */}
                  {contact?.socialLinks && Object.keys(contact.socialLinks).some(key => contact.socialLinks[key]) && (
                     <Card>
                        <CardHeader>
                  ...
                  {/* Recent Notes */}
                  {activity?.notesUploaded && activity.notesUploaded.length > 0 && (
                     <Card>
                        <CardHeader>
                           <CardTitle className="flex items-center gap-2">
                              <BookOpen className="h-5 w-5" />
                              Recent Notes ({activity.notesUploaded.length})
                           </CardTitle>
                        </CardHeader>
                        <CardContent>
                           <div className="grid gap-4">
                              {activity.notesUploaded.slice(0, 6).map((note) => (
                           ...
                           {activity.notesUploaded.length > 6 && (
                              <div className="mt-4 text-center">
                                 <Button variant="outline" size="sm">
                                    View All Notes ({activity.notesUploaded.length})
                                 </Button>
                              </div>
                           )}
                        </CardContent>
                                    ...
                                    <div key={follower._id} className="flex items-center gap-3">
                                       <Avatar className="w-8 h-8">
                                          ...
                                          <AvatarFallback className="text-xs">
                                             {follower.profile?.firstName?.[0]}{follower.profile?.lastName?.[0]}
                                          </AvatarFallback>
                                       </Avatar>
                                       <div>
                                          <p className="font-medium text-sm">{follower.profile?.firstName} {follower.profile?.lastName}</p>
                           ...
                           <CardContent>
                              <div className="space-y-3">
                                 {activity.following.slice(0, 5).map((followedUser) => (
                                    <div key={followedUser._id} className="flex items-center gap-3">
                                       <Avatar className="w-8 h-8">
                                          <AvatarImage src={followedUser.profile?.avatar} />
                                          <AvatarFallback className="text-xs">
                                             {followedUser.profile?.firstName?.[0]}{followedUser.profile?.lastName?.[0]}
                                          </AvatarFallback>
                                       </Avatar>
                                       <div>
                                          <p className="font-medium text-sm">{followedUser.profile?.firstName} {followedUser.profile?.lastName}</p>
                                          <p className="text-xs text-gray-600">@{followedUser.username}</p>
                                       </div>
                                    </div>
                                 ))}
                              </div>
