...
const NotesFeedPage = () => {
   ...
   const clearSpecificFilter = useCallback((filterKey) => {
      if (filterKey === 'search') {
         ...
         const defaultValue = (filterKey === 'category' || filterKey === 'difficulty') ? 'all' : ''
   ...
   const renderPagination = () => {
      if (!feedData?.pagination || feedData.pagination.totalPages <= 1) return null

      const { currentPage: page, totalPages, hasNextPage, hasPrevPage } = feedData.pagination
               ...
               <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                  <div>
                     ...
                     <p className="text-gray-600 mt-1">
                        Discover and download study materials from your academic community
                     </p>
                  </div>
                        ...
                        key !== 'sortBy' && key !== 'sortOrder' && value && value !== 'all'
                           ...
                           <div className="flex items-center gap-2 flex-wrap">
                              ...
                              {Object.entries(filters).map(([key, value]) => {
                                 if (key === 'sortBy' || key === 'sortOrder' || !value || value === 'all') return null
                                 return (
                                    <Badge
                                       key={key}
                                       variant="secondary"
            ...
            {feedData && (
               <div className="flex items-center justify-between mb-6">
                  <div className="text-sm text-gray-600">
                     Showing <span className="font-medium">{feedData.notes?.length || 0}</span> of{' '}
                     <span className="font-medium">{feedData.pagination?.totalItems || 0}</span> notes
                  </div>
                  <div className="text-sm text-gray-600">
                     Page <span className="font-medium">{feedData.pagination?.currentPage || 1}</span> of{' '}
                     <span className="font-medium">{feedData.pagination?.totalPages || 1}</span>
                  </div>
               </div>
            )}
                  ...
                  <div className="max-w-md mx-auto">
                     <div className="text-red-600 mb-6">
                        ...
                        <p className="text-sm text-gray-600 mb-4">
                           {error?.message || error?.data?.message || 'Something went wrong while fetching notes. Please check your connection and try again.'}
                        </p>
                        {error?.status && (
