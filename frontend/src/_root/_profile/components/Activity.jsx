   ...
   return (
      <div className='space-y-6'>
            ...
            {statsData?.success && statsData.data.categoryStats.length > 0 && (
               <div className='bg-white rounded-lg shadow-lg p-6'>
                  ...
                  <div className='space-y-3'>
                     {statsData.data.categoryStats.slice(0, 5).map((category, index) => (
                        <div key={index} className='flex items-center justify-between p-3 bg-gray-50 rounded-lg'>
                           <div>
                              <p className='font-medium capitalize'>{category._id.replace('-', ' ')}</p>
                              <p className='text-sm text-gray-600'>{category.count} notes</p>
                           </div>
                           <div className='text-right'>
                              <p className='text-sm font-medium text-green-600'>{category.totalDownloads} downloads</p>
                              <p className='text-xs text-gray-500'>
                                 {category.count > 0 ? (category.totalDownloads / category.count).toFixed(1) : '0'} avg
                              </p>
                           </div>
                        </div>
                     ))}
         ...
         {statsData?.success && statsData.data.recentActivity.length > 0 && (
            <div className='bg-white rounded-lg shadow-lg p-6'>
               <h3 className='text-lg font-semibold text-gray-900 mb-4'>Recent Uploads (Last 30 Days)</h3>
               <div className='space-y-3'>
         ...
         <div className='grid grid-cols-1 lg:grid-cols-2 gap-6'>
            ...
            <div className='bg-white rounded-lg shadow-lg p-6'>
               ...
               ) : followersData?.success && followersData.data.followers.length > 0 ? (
            ...
            <div className='bg-white rounded-lg shadow-lg p-6'>
               ...
               ) : followingData?.success && followingData.data.following.length > 0 ? (
                  <div className='space-y-3 max-h-64 overflow-y-auto'>
                     {followingData.data.following.slice(0, 5).map((following) => (
                        <UserCard key={following._id} user={following} type='following' />
                     ))}
            ...
            <div className='bg-white rounded-lg shadow-lg p-6'>
               ...
               <div className='grid grid-cols-1 md:grid-cols-3 gap-6'>
                  <div className='text-center p-4 bg-blue-50 rounded-lg'>
                     ...
                     <h4 className='font-semibold text-gray-900'>Upload Consistency</h4>
