...
import GoogleDriveStatus from '@/components/google/GoogleDriveStatus'
import {
  useGetGoogleDriveAccountInfo,
  useGetGoogleDriveFolderStructure,
...
const Drive = () => {
  // Get Google Drive token from localStorage (same as UploadNote component)
  ...
  } = useGetGoogleDriveFolderStructure(!!googleDriveToken)

  const loading = isFolderLoading
  const error = folderError
  const needsReauth = error?.needsReauth || false
  const driveData = folderData

  // Handle connection status changes from GoogleDriveStatus
  // Use useCallback to prevent infinite re-renders
  const handleConnectionChange = useCallback(status => {
    setConnectionStatus(status)
  }, [])

  const accountInfo = connectionStatus.accountInfo
    ...
    <div className='space-y-6'>
      ...
      {error && connectionStatus.isConnected && (
        <Alert variant='destructive'>
          ...
          <AlertDescription>
            {error?.message || 'Failed to load folder structure'}
          </AlertDescription>
        </Alert>
      )}
        ...
        <Card>
          <CardContent className='pt-6'>
            <div className='text-center py-12'>
              <HardDrive className='h-16 w-16 mx-auto text-muted-foreground/50 mb-4' />
              <p className='text-muted-foreground'>
                Connect your Google Drive to view folder structure and storage
                stats
              </p>
            </div>
          </CardContent>
